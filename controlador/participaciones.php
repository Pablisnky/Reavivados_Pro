			...
			<body>
				...
				<div class="Secundario" onclick="ocultarMenu()">
					...
					if($Participaciones == 0){ ?> 	
			                ...
			                <table>
			                    ...
			                    <tbody>
				                    <?php
									    ...
									    $Consulta= "SELECT ID_Prueba, ID_PP, Puntos, Fecha_pago, Tema, reavi_capitulo.capitulo FROM participantes_pruebas INNER JOIN reavi_capitulo ON participantes_pruebas.Fecha_pago=reavi_capitulo.fecha WHERE ID_Participante = '$participante' AND Tema = 'Reavivados' AND Prueba_Cerrada = 1 ORDER BY Fecha_pago DESC";
									    $Recordset=mysqli_query($conexion, $Consulta); 					            		
				                        while($Pruebas= mysqli_fetch_array($Recordset)){  
					                        ...
					                        <tr class="tabla_10">
												...
												<td class="tabla_7">
													<?php echo date("d-m-Y", strtotime($Pruebas["Fecha_pago"])); ?>
												</td>
												<?php
												...
												<td class="tabla_2">
													<a href="../vista/MostrarDetalleTest.php?ID_Participante=<?php echo $participante?>&Tema=<?php echo $Pruebas['capitulo'];?>&Capitulo=<?php echo $Pruebas['capitulo'];?>&ID_Prueba=<?php echo $Pruebas['ID_Prueba'];?>&codigoPrueba=<?php echo $Pruebas['ID_PP']?>">Ver</a>
												</td> <?php 
					                         	$i++;     ?>			                        	
											 </tr>	<?php
					                    }   
						...
						<div class="participaciones">
			                <table>
								<caption>Libros biblicos</caption>
			                    <thead>
			                        <tr>
			                            <th></th>
			                            <th>FECHA</th>
			                    ...
			                    <tbody>
				                    <?php
				                        $i = 1;
				                        //se consulta las pruebas en las que el participante ha participado
									    $Consulta= "SELECT ID_Prueba, ID_PP, Puntos, Fecha_pago, Tema FROM participantes_pruebas WHERE ID_Participante = '$participante' AND Tema != 'Reavivados' AND Prueba_Cerrada = 1 ORDER BY Fecha_pago DESC";
									    $Recordset=mysqli_query($conexion, $Consulta); 					            		
				                        while($Pruebas= mysqli_fetch_array($Recordset)){ 		
					                        //Se cambia el formato de los puntos, la Pruebase decimal es recibida con punto desde la BD y se cambia a coma
					                    	$Decimal = str_replace('.', ',', $Pruebas["Puntos"]); ?>
					                        <tr class="tabla_10">
												...
												<td class="tabla_7">
													...
													<td class="tabla_8">
														<?php echo $Pruebas["Tema"];?>
													</td>											
					                            <td class="tabla_9">
													<?php echo $Decimal;?> 
												</td> 
												<td class="tabla_2">
													<a href="../vista/MostrarDetalleTest.php?ID_Participante=<?php echo $participante?>&Tema=<?php echo $Pruebas['Tema'];?>&ID_Prueba=<?php echo $Pruebas['ID_Prueba'];?>&codigoPrueba=<?php echo $Pruebas['ID_PP']?>">Ver</a>									
												</td> <?php 
					                         	$i++;     ?>			                        	
											 </tr>	<?php
					                    }   
